# Hypothesis Preregistration: ETHUSDT 15m volatility-filtered momentum (long-only)
...
- title: ETHUSDT 15m volatility-filtered momentum (long-only)
- author / session: gmaq continuation session
- date (UTC): 2026-08-16

## 2. Hypothesis (required)

- Investible claim: a positive 1-hour return is followed by a positive next-hour return only when recent realized volatility is below its own median; after conservative costs the filtered signal outperforms unfiltered momentum.
- Mechanism: momentum continuation is more reliable in calm microstructure.
...
- filter: realized volatility (stdev of 1-bar returns over the last 24 candles) below the median of the same rolling volatility over the last 96 values
- entry: long 100 USDT notional at the open of candle T+1 when the 1-hour return over candles T-3..T is > 0 AND the volatility filter passes

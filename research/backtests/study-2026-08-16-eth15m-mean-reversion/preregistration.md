# Hypothesis Preregistration: ETHUSDT 15m mean reversion (long-only)
...
- title: ETHUSDT 15m mean reversion (long-only)
- author / session: gmaq continuation session
- date (UTC): 2026-08-16

## 2. Hypothesis (required)

- Investible claim: after a negative 1-hour return (4 x 15m candles) on ETHUSDT perpetual, the following 1-hour return is positive on average, after conservative costs.
- Mechanism: short-horizon overreaction: a down hour is followed by partial rebound.
...
- entry: long 100 USDT notional at the open of candle T+1 when the return over candles T-3..T is < 0

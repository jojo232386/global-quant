# Hypothesis Preregistration: ETHUSDT 15m UTC session effect (long-only)
...
- title: ETHUSDT 15m UTC session effect (long-only)
- author / session: gmaq continuation session
- date (UTC): 2026-08-16

## 2. Hypothesis (required)

- Investible claim: holding ETHUSDT long only inside 00:00-08:00 UTC earns a positive average return after conservative costs, relative to the full-day baseline.
- Mechanism: concentrated liquidity and directional flow during the European morning session.
...
- The session effect is considered dead if its OOS return is not better than buy-and-hold over the same window.
...
- entry: long 100 USDT notional at the open of candle T+1 when the calendar hour of that bar's open time is in [0, 8) UTC and no position is open

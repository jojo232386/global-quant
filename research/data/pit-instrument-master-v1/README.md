...
activity evidence: 78 members have gap-free coverage through 2023-12-31;
`BZRXUSDT` and `YFIIUSDT` use the existing official announcement plus
aggregate-trade terminal evidence. `AKROUSDT` is outside the cohort and remains
explicitly quarantined because its official notice conflicts with later
official aggregate trades.
...
Price event timestamps support historical activity, but current
...
`[2021-01-04T19:51:02.039Z, 2024-01-01T00:00:00Z)`. It returns only this fixed

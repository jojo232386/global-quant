# NT-GATE-1B v1.0 Frozen Protocol
...
credential use, or Demo order is permitted before that commit.
...
- No Binance production or legacy Testnet connection.
...
Any production, Spot, COIN-M, Testnet, custom, redirected, or unrecognized host
is a safety failure. The runner must require an explicit
...
1. Demo environment and allowlisted endpoints are resolved from the pinned
...
- maximum submitted orders for the entire gate: `20`;
- maximum one-way net position per instrument: one;
- protection stop and take triggers remain the shared Strategy defaults;
...
3. Confirm separate stop-market and market-if-touched protection orders.
...
If Demo liquidity does not produce a partial fill or the funding boundary is
unavailable within the frozen deadline, the result is `INCONCLUSIVE`, not PASS
and not an invitation to alter the protocol.
...
minimum above the frozen cap, no partial fill, or no funding boundary within
the deadline. It forbids Gate 2 and requires a separately authorized retry.

`STOP` applies to any safety or engineering failure, including:

- any production/Testnet/custom endpoint or real credential exposure;
...
- The pinned adapter resolves USD-M Demo HTTP and stream traffic to
  `demo-fapi.binance.com` and `demo-fstream.binance.com` respectively.

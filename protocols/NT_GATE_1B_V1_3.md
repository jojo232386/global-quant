# NT-GATE-1B v1.3 Frozen Protocol
...
credential use, or Demo order is permitted before that commit. Version `1.2`
remains permanently closed as `INCONCLUSIVE/MISSING_DEMO_CREDENTIALS` at tag
`nt-gate-1b-v1.2-inconclusive`. Version `1.3` retries only the missing Demo
execution evidence; it does not reinterpret or overwrite v1.2. Version `1.0`
...
- The credential prompt module and its thin entrypoint are also SHA-256 bound.
  They may only inject credentials into the current process for one signed
  preflight; they may not persist or forward them through arguments or the
...
The parent process environment must contain none of the Demo, live, or Testnet
Binance credential variables. `scripts/run_gate_1b_prompted.py` requires an
interactive terminal and hiddenly prompts for the Demo key and secret. It
constructs an in-process mapping containing only `BINANCE_DEMO_API_KEY` and
`BINANCE_DEMO_API_SECRET`, passes that mapping directly to the signed preflight,
then clears it. It must refuse before prompting if any Binance credential
variable is already present.

The underlying runner retains its fail-before-network checks and must reject
missing or empty Demo values, any of `BINANCE_API_KEY`, `BINANCE_API_SECRET`,
`BINANCE_TESTNET_API_KEY`, `BINANCE_TESTNET_API_SECRET`,
`BINANCE_FUTURES_TESTNET_API_KEY`, or
`BINANCE_FUTURES_TESTNET_API_SECRET`, and a missing explicit arming flag.

Secrets must never appear in environment exports, arguments, configuration
files, logs, evidence, tracebacks, process listings, Git objects, hashes,
screenshots, clipboard captures, or reports. The prompt passes them directly
from process memory into Nautilus configuration. Evidence records only presence
booleans and redaction status. Neither credential value nor account identifier
...
`--confirm-demo-only` arming flag in addition to an interactive prompt and
valid Demo values.
...
- Immediately after the protocol commit and annotated tag, run only the
  interactive, read-only signed account preflight. Do not start the Demo node
  or submit an order unless preflight returns PASS and the sanitized evidence
  passes review.
...
- The logged-in Demo trading and Demo API Management interface is hosted at
  `https://demo.binance.com/`; this web portal is not an execution endpoint.

# NT-GATE-1B v1.6 Demo Mutation Protocol Candidate
...
Proposed final tag: `nt-gate-1b-v1.6-protocol`

This candidate authorizes no credential access, authenticated request, Demo mutation,
order, cancel, fill, account-setting change, or production connection. It is a design
candidate awaiting explicit user approval. The final protocol must be committed and
annotated-tagged before any credential-bearing implementation or runtime session.
...
The following are absent: `priceMatch`, `closePosition`, `stopPrice`, `activationPrice`,
`callbackRate`, `workingType`, `priceProtect`, `goodTillDate`, and modify fields. No
self-trade-prevention behavior is claimed for `GTX`.
...
After offline tests and self-review pass, stop at
`PASS_READY_FOR_USER_PROTOCOL_APPROVAL`. If the user approves OPTION_A, create the final
v1.6 protocol commit and annotated tag, then implement/test the thin runner. Do not enter a
credential-bearing or mutating session without another explicit authorization.
